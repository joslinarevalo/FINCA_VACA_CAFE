...
if (isset($_POST['validar_campos']) && $_POST['validar_campos']=="si_por_campo") {
		...
		$array_seleccionar['campo']="int_idexpediente";

		if ($_POST['tipo']=="nombre_bovino_guardar") {
			$array_seleccionar['nva_nom_bovino']=$_POST['campo'];
		}else{}
		...
		if ($resultado[0]==0 && $resultado[4]==0) {
			...
			$campo_existe = "si_existe";
			print json_encode(array("Error",$resultado,$array_seleccionar,$campo_existe));
	...
	//$nombre = $sql->cubrid_real_escape_string($_POST['nom_bovino']);
		$sql = "SELECT * FROM tb_expediente WHERE nva_nom_bovino='$_POST[nom_bovino]'";
		$resultadito = $modelo->get_query($sql);
		if ($resultadito[0] == '1') {
		print json_encode(array("Ya existe", $resultadito));
		exit();
	} else{
