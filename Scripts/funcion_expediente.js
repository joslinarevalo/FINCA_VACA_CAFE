...
$(function () {
	...
	$(document).on("blur", ".validar_campito", function (e) {
		e.preventDefault();
		...
		$.ajax({
	    	...
	    	Toast.fire({
			        icon: 'error',
			        title: 'Este nombre del bovino ya existe'
		    	});
		    	return;

	    		$('#campo_igual').val(json[3]);
	...
	$(document).on("submit", "#formulario_registro", function (e) {
		...
		if ($("#tipo_bovino").val() == "Seleccione"){
 		...
 		if ($("#propietario").val() == "Seleccione"){
			...
			return;
 		}if ($("#campo_igual").val() == "yapaso"){
 			Toast.fire({
		        icon: 'info',
		        title: 'Bovino ya existe,Escriba otro nombre'
		    });
			return;
 		}
 		var campo_igual = $("#campo_igual").val();
